...
const VPSPricingCard = ({ planName, price, url, features, lang }) => {
  ...
  const [VPSInput, setVPSInput] = useState("");
  ...
  const handlePurchase = (e) => {
    if (!isModalOpen) {
      e.preventDefault();
      openModal();
    } else if (VPSInput.length == 0) {
      e.preventDefault();
      setIsInputEmpty(true);
    } else if (!/^[A-Za-z\s]+$/.test(VPSInput)) {
      e.preventDefault();
  ...
  return (
    ...
    >
      ...
      <div className="max-xl:w-full">
        ...
        {/* Buy Button */}
        <a
          href={`${url}${VPSInput && `&vps=${VPSInput.replace(" ", "-")}`}`}
      ...
      {isModalOpen ? (
        <motion.form
          className="w-full lg:w-[400px] mx-auto"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-start font-bold text-[18px] sm:text-[22px]">
              {lang == "en" ? "VPS Name" : "اسم الخادم"}{" "}
              {isInputEmpty && <span className="text-error">*</span>}
            </h4>
            <IoCloseSharp
          ...
          <input
            type="text"
            placeholder={
              lang == "en" ? "Enter a name for your VPS" : "ادخل اسم لخادمك"
            }
            onChange={(e) => {
              setVPSInput(e.target.value);
            ...
            border-[2px] ${
              isInputEmpty || isInputNotEnglish
                ? "border-error"
                : "border-primary"
            }
        ...
        >
          ...
          {features?.speed && (
            <div className="flex items-center gap-4">
              ...
              <span>
                {lang == "en" ? features.speed[0] : features.speed[1]}
              </span>
            </div>
          )}
          {features?.ip && (
